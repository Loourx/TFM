...
Este trabajo de fin de máster propone el concepto de servicio de movilidad nocturna para el transporte nocturno de trabajadores de turno en el corredor del Complexo Hospitalario Universitario de Ferrol (CHUF, Avenida de la Residencia s/n, Ferrol) hacia las zonas residenciales de la ciudad, operando en la franja horaria comprendida entre las 22:00 y las 06:00~h; el horizonte temporal se determina en función de la tecnología seleccionada en la fase de diseño.
...
alternos que incluyen la noche \parencite{SanchezSellero2025}. Entre los
...
transporte colectivo ha cesado \parencite{URBACT2025Gender}. Para todos
...
sin cobertura posterior \parencite{dAntonio2024}. La escala del problema en
...
El argumento central de este trabajo es que el servicio de movilidad nocturna no es una solución pensada para metrópolis: es estructuralmente adecuado para corredores
...
El \textbf{objetivo general} de este trabajo es diseñar un concepto de servicio de movilidad nocturna para trabajadores sanitarios en el corredor CHUF–Ferrol, fundamentado en evidencia empírica propia, y extraer de él un modelo de servicio transferible a ciudades europeas con perfil análogo.
...
\begin{enumerate}[label=\arabic*., leftmargin=*, noitemsep]
  ...
  \item Construir el perfil del usuario principal (personal sanitario de turno nocturno del \textsc{chuf}) a partir de los datos primarios de la encuesta propia (n=167), con literatura europea como marco de contextualización.
  \item Documentar empíricamente las barreras de movilidad nocturna del personal sanitario del CHUF mediante encuesta propia (n=167).
...
La memoria se organiza en nueve capítulos.

El \textbf{capítulo 2} establece el marco contextual: el análisis \textsc{pestel} de la movilidad nocturna en Europa y el mapa de \textit{stakeholders} del ecosistema. Es el capítulo que sitúa el problema en su dimensión política, económica y social.

El \textbf{capítulo 3} contiene la investigación de usuario: los perfiles de usuario construidos a partir de datos primarios de la encuesta propia, el análisis cuantitativo y cualitativo de hallazgos y el \textit{benchmarking} de servicios de movilidad nocturna.

El \textbf{capítulo 4} recoge los criterios de diseño del servicio: los insights de diseño derivados de la investigación, la traducción en requisitos mediante \textsc{qfd} y \textsc{moscow} y la definición de los valores del servicio.

El \textbf{capítulo 5} desarrolla la propuesta de servicio: la evaluación de candidatos y selección justificada, el \textit{service blueprint}, el \textit{journey map}, los principios de diseño del espacio del servicio y el modelo de negocio conceptual.

El \textbf{capítulo 6} define el modelo de replicabilidad: el análisis de representatividad de Ferrol y los parámetros del modelo transferible a corredores europeos análogos.

El \textbf{capítulo 7} presenta la evaluación del proyecto: el pliego de condiciones, la matriz de afirmaciones--evidencia y el análisis de viabilidad y riesgos.

El \textbf{capítulo 8} recoge las conclusiones: síntesis de hallazgos, limitaciones del trabajo y líneas futuras.

El \textbf{capítulo 9} detalla el presupuesto del proyecto como ejercicio de cuantificación del coste real del trabajo de diseño.